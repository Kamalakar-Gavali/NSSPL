...
app.post("/login", async (req, res) => {
  ...
  if (!nullOrUndefined(response)) {
    ...
    res
      .status(401)
      .send({ msg: `${req.body.email} dosent' exist,Signup first` });
  }
});
app.post("/addTask", async (req, res) => {
  ...
  if (nullOrUndefined(response)) {
    ...
    res
      .status(201)
      .send({ ok: true, msg: "Task Added Successfully", newData: data });
  } else {
...
app.post("/users", async (req, res) => {
  const response = await userModel.find({
    groupId: { $gte: Number(req.body.groupId) },
  });
  console.log(response);
  res.send(response);
});
...
app.put("/tasks", async (req, res) => {
  const data = await TaskModel.findByIdAndUpdate(
    req.body.id,
    { assignedTo: req.body.newAssignedUser },
    (err, result) => {
      console.log(result);
    }
  );
  ...
  res
    .status(200)
    .send({
      msg: `Task is taken away from ${req.body.prevUserEmail} and Assigned To ${req.body.newAssignedUser}`,
    });
});
app.get("/*", (req, res) => {
